...
export function TransactionTable({ transactions, accountId }) {
  ...
  const handleSelectAll = () => {
    setSelectedIds((curr) =>
      curr.length === paginatedTransactions.length ? [] : paginatedTransactions.map((t) => t.id)
    );
  };
            ...
            {paginatedTransactions.length === 0 ? (
              ...
              paginatedTransactions.map((t) => (
                <TableRow key={t.id} className="border-slate-800/50 hover:bg-slate-800/40 transition-colors group">
                  ...
                  <TableCell className={cn("py-2.5 text-right font-mono text-sm",
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent className="bg-slate-900 border-slate-700 text-slate-200 font-mono text-xs">
                            ...
                            <p>{t.nextRecurringDate ? format(new Date(t.nextRecurringDate), "dd MMM yyyy") : "N/A"}</p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    ) : (
                  ...
                  <TableCell className="py-2.5 text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="bg-slate-900 border-slate-700 font-mono text-xs shadow-xl min-w-[120px]">
                        <DropdownMenuItem
                          onClick={() => navigate(`/transaction/create?edit=${t.id}`)}
